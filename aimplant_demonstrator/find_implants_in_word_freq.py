items = set()
glossary_file = '/home/abragam23/fedhealth_data/Glossary_only_known_implants.txt'
with open('word_freq_summary_085.txt', 'r', encoding='utf-8') as f:
    with open(glossary_file, 'r', encoding='utf-8') as g:
        glossary = [line.strip() for line in g if line.strip()]
        print(f"Loaded {len(glossary)} items from {glossary_file}")
        for line in f:
