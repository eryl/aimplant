    ...
    with torch.inference_mode():
        while True:
            ...
            if query_parts and query_parts[0] == '/quit':
                ...
                all_words, word_indices, word_vectors = compute_embeddings(model, query, layers=layers, aggregation=aggregation, stop_list=stop_list)
                ...
                for word, neighbours in query_result:
                    print(f"Word: {word}")
                    for distance, neighbour_word, word_class in neighbours:
                        print(f"  Neighbour: {neighbour_word}, Distance: {distance:.4f}, Class: {word_class}")
            else:
