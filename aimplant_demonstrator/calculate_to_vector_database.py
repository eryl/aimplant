...
def main():
    ...
    database_path = output_dir / 'lancedb'

    with torch.inference_mode(), DataBaseHandler(database_path, chunk_size=args.chunk_size) as vectordb:
        model.eval()
...
class DataBaseHandler:
    def __init__(self, database_path: Path, chunk_size=128):
        self.words = []
    ...
    def add_data(self, labeled_word: tuple[int, str], vectors: dict[str, np.ndarray]):
        self.words.append(labeled_word)
        for vector_name, word_vector in vectors.items():
            self.vectors[vector_name].append(word_vector)
        if len(self.words) >= self.chunk_size:
            self.sync_vectors()
    
    def sync_vectors(self):
        if len(self.words) > 0:
            for vector_name, vectors in self.vectors.items():
